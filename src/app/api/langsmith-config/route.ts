...
export async function GET(request: NextRequest) {
  try {
    // Explicit disable: set OTEL_DISABLED=true (or 1) to turn off OTEL (no key sent → client skips init)
    const otelDisabled = process.env.OTEL_DISABLED;
    if (otelDisabled === 'true' || otelDisabled === '1') {
      ...
      return NextResponse.json({ apiKey: undefined, endpoint, project });
    }
