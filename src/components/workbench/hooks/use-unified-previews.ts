...
function getPreviewTitle(toolName: string, request: any): string {
  switch (toolName) {
    ...
    case "phase_artifact_from_upload": {
      const artifactType = (request.args?.artifact_type as string)?.toLowerCase();
      ...
      if (artifactType === "project")
        return `Create project from document: ${request.args?.project_name || request.preview_data?.project_name || request.args?.project_id || "Unknown"}`;
      return `Phase artifact from upload: ${request.args?.artifact_type || "Unknown"}`;
    }
